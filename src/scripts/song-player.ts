 ...
 *
 *   <div class="song-player" data-src="音频直链" data-title="歌名" data-artist="歌手" data-cover="封面图URL"><a href="音频直链">♪ 播放音频</a></div>
 *
 * - data-src 必填（http(s) 或协议相对）；data-title / data-artist / data-cover 可选；
 * - 占位块内的文本是无 JS 时的降级内容（渲染时被替换）；
 * - 脚本把占位块增强为统一风格的播放条：封面 / 歌名 / 播放暂停 / 进度拖动 / 时间；
 * - 同页多个播放条共享一个 Audio 实例，同一时间只播放一个。
 *
 * 由文章页 <script data-astro-rerun> 引入，每次 View Transitions 导航后重新执行。
 */
