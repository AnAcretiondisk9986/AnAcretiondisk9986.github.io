...
(function (global) {
  ...
  // Expose the library on the global scope.
  // 原 UMD 分支(AMD / CommonJS / global)在 ESM 打包环境下会导致
  // Vite 无法静态分析而残留 import,这里统一改为 globalThis 挂载,
  // 并在文件末尾提供 ESM 默认导出(见文末 export default)。
  ;(typeof globalThis !== 'undefined' ? globalThis : window).WordCloud = WordCloud
})(globalThis) // jshint ignore:line

export default (typeof globalThis !== 'undefined' ? globalThis : window).WordCloud
