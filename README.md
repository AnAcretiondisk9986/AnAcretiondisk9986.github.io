...
- **全站背景图**：非首页页面（卷册目录 / 图志 / 留言 / 关于）与首页 hero 共用同一背景图——直接引用 `frontend.json` 的 `stillHeroImage` / `fluidHeroImage`（含焦点位置），管理面板「前端定制」改图即全站同步；`fixed + z-index:-1` 置于纸纹层之下，带与 hero 同款 scrim 遮罩保证文字可读性；仅 Liquid 主题（静澈 / 流形）显示，留白 / 行迹经典主题保持纸色原样，首页由 hero 自备背景不重复叠加。
...
- 验证：`npm run build` 21 页成功；headless 实测 4 页面 × 2 Liquid 主题背景层显示、首页与 legacy 主题隐藏、边缘高光像素亮度差异（暗色 89 vs 内部 32）均符合预期；admin-server 语法检查通过。
