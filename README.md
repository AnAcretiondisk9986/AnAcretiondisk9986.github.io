...
- **正文格式化工具栏**：编辑器正文上方新增一排快捷按钮——加粗 / 斜体 / 删除线 / 下划线 / 高亮（H，`<mark>`）、二级标题 / 引用 / 链接 / 图片 / 列表 / 分割线 / 代码块；**色板 + 调色盘**给选中文字上色（包 `<span style="color:…">`）；**居左 / 居中 / 居右**（包 `<div style="text-align:…">`，重复点击替换不嵌套）。实时预览支持白名单 HTML 渲染（变色 / 居中 / 高亮等即时可见；含 script / on* 的标签拒绝渲染回退文本）。线上 `.prose mark` 高亮样式已适配明暗主题。
