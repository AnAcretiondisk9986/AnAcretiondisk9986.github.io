...
- **管理面板支持拖拽 / 粘贴 / 选择上传音频**：自动解析 mp3 / flac 等文件的歌名、歌手、内嵌封面（music-metadata + sharp 转 WebP 存图片目录），解析成功直接插入播放条到光标处；无法解析时弹窗让用户手动补全。
- **外链可播性自动探测**：「♪ 音乐」对话框粘贴链接后自动探测（后端 `/api/audio-probe`：HEAD / Range 校验 Content-Type 与音频魔数），显示「✓ 可播放」或失败警告，避免插入无效链接。
- 网易云官方外链播放器（iframe）容器按 `src` 的 height 参数适配尺寸（单曲 66px / 歌单 90px），不再被 16:9 规则撑出空白。
- 管理面板「♪ 音乐」按钮：粘贴音频直链 / 网易云歌曲页链接（可选填歌名 / 歌手 / 封面）或已有的 song-player 占位代码，一键生成并插入；实时预览同步支持占位块渲染（原生 audio 可试听）。
- **正文格式化工具栏**：编辑器正文上方新增一排快捷按钮——加粗 / 斜体 / 删除线 / 下划线 / 高亮（H，`<mark>`）、二级标题 / 引用 / 链接 / 图片 / 列表 / 分割线 / 代码块；**色板 + 调色盘**给选中文字上色（包 `<span style="color:…">`）；**居左 / 居中 / 居右**（逐行包 `<div style="text-align:…">`，保留多行换行，重复点击替换不嵌套）。实时预览支持白名单 HTML 渲染（变色 / 居中 / 高亮等即时可见；含 script / on* 的标签拒绝渲染回退文本）。线上 `.prose mark` 高亮样式已适配明暗主题。
- **图片尺寸设置**：工具栏新增「⛶ 图宽」按钮——自动提取光标处的图片（`![alt](url)`），弹窗设置宽度（百分比 / 像素，或点「原图」恢复不限制），生成 `<img src="…" style="width:…">` 替换；预览白名单同步支持自闭合 `<img>` 标签渲染。
- **编辑器体验升级**：正文编辑区高度提升至 `min(72vh, 860px)`；**渲染区可直接编辑**（contenteditable，改动经 turndown 转回 Markdown 写回源码区，同步不打断预览光标；粘贴自动转纯文本）；**源码区与渲染区滚动联动**（点击 / 按键时光标行 ↔ 预览滚动位置按比例互相同步）；**分栏宽度可拖拽调节**（源码 / 渲染 20%–80%，localStorage 记忆）。turndown 以本地文件引入（`admin/vendor/turndown.js`，面板离线可用）。

- 样式：`global.css` 卡片样式走全局变量自动适配明暗；minimal 主题卡片化、liquid 主题沿用 `--liquid-radius`；移动端自动换行。
- 模板仓库已同步（`blog-template` 同版本，音频存其 `public/audio/`）。
