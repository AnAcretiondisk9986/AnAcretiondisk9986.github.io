...
- 文案覆盖：全站导航与页脚、首页主标题 / 操作 / 最新文章、文章目录与详情、词云、照片页、关于页外观标题、留言 / 文章评论动态状态、404；文章正文和 `about.json` 中用户可编辑内容不受影响。
- 动态文本：留言和评论的占位、加载 / 空态 / 提交状态按当前主题生成；照片排序后同步更新两套编号，留白显示「图片 / 照片」，行迹保留 `FOLIO / INDEPENDENT`，查看器按钮与来源链接同样适配。
- 用户反馈修复：主题文案使用嵌套 `span` 后，极简导航原有的宽泛选择器误将五个标签一并隐藏；现改为 `.nav-index` 精确隐藏罗马序号，导航文字恢复。极简「全部文章」列表隐藏 `FOLIO / 文章` 后缀，仅保留 `01 / 02 / …`，行迹主题仍保留 `FOLIO`。
- 验证：`npm.cmd run build` 成功生成 20 页；浏览器实测留白 / 行迹切换、五项主导航、390×844 无溢出、文章纯序号、照片排序后编号、留言动态文案均通过，控制台无 warning / error。

**🎨 全站极简重构 + 双视觉主题（留白 / 行迹）**

- 主题架构：保留重构前完整视觉为「行迹」，新增全站极简视觉「留白」并设为默认；视觉主题使用 `data-visual-theme` 与 `localStorage('visual-theme')` 持久化，和原有 `data-theme` 明暗模式相互独立，均适配 Astro View Transitions 导航。
- `src/layouts/BaseLayout.astro`：引入 `src/styles/minimal-theme.css`；页面首帧同步恢复视觉主题，避免闪烁；明暗按钮按当前状态显示太阳 / 月亮并更新无障碍标签。
- `src/pages/about.astro`：新增「界面主题」分段选择器，可在「留白 / 行迹」之间即时切换；按钮使用 `aria-pressed`，状态变化通过 `aria-live` 提示，选择跨页面保持。
- `src/styles/minimal-theme.css`（新增）：在不改页面数据结构和脚本契约的前提下覆盖首页、卷册、文章、图志、关于、留言、文章评论与页脚；采用系统字体、平面中性色、克制红 / 绿强调色、半透明吸顶导航、紧凑分段控件与低干扰反馈；支持明暗、`prefers-reduced-motion`、`prefers-reduced-transparency`、`prefers-contrast`。
- 功能保持：卷册排序、词云、图志 tab / 排序 / 视图 / 分页 / 22 图轮播 / 大图查看器、文章导出 PDF、Waline 留言和文章评论均保留；词云改为读取当前主题字体变量，并在视觉主题切换后自动重绘。
- 移动端：390×844 下首页首屏保留下一节提示；导航、图志和主题控件无横向溢出；文章侧栏压缩为返回 / 导出 + 两列元数据，移动端隐藏侧栏词云，使正文标题进入首屏。
- 验证：`npm run build` 成功生成 19 页；浏览器实测 1280×720 与 390×844，双视觉主题切换、明暗切换、跨页持久化、真实 VT 导航、卷册排序、图志轮播 / 查看器 / 翻页全部通过；控制台 0 warning / error。
